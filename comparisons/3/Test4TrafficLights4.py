...
# the light with biggest awaiting time queue is turned on after 20s
...
vehicles_queues = [[], [], [], []]
vehicles_queue_time = [[], [], [], []] # stores the time the vehicle is waiting in the red light queue (currently)
...
class CentralCoordinationAgent(Agent):
    ...
    class CentralCoordinationBehav(CyclicBehaviour):
        async def on_start(self):
            ...
            # queue if the biggest medium await time, so, 
        ...
        async def run(self):            
            max_awaiting_time_queue = -1
            max_medium_time = 0
            for i in range(len(vehicles_queue_time)):
                if len(vehicles_queue_time[i]) > 0:
                    medium_time = sum(vehicles_queue_time[i])/len(vehicles_queue_time[i])
                    if medium_time > max_medium_time:
                        max_medium_time = medium_time
                        max_awaiting_time_queue = i + 1
                        # ^ this stores the traffic light with the biggest medium awaiting time (+1 because list index starts with 0)

            if(max_awaiting_time_queue == 1 or max_awaiting_time_queue == 3):
            ...
            elif(max_awaiting_time_queue == 2 or max_awaiting_time_queue == 4):
                    ...
                    if light_color == "Green": 
                        ...
                        for i in range(len(position_ligths)):
                            if self.agent.traffic_light.number == i+1: # if light 1, 2, 3, 4
                                ...
                                if len(vehicles_queue_time[i]) != 0:
                                    vehicles_queue_time[i].pop(position_in_list) # takes the time off the waiting list 
                        ...
                        if count == 1: # the first time it reaches the red light
                            for i in range(len(position_ligths)):
                                if self.agent.traffic_light.number == i+1: # if light 1, 2, 3, 4
                                    ...
                                    # stores the time that the vehicle is currently waiting on the red light
                                    vehicles_queue_time[i].append(round(self.agent.waiting_time, 3))
                                    # the position in the list will be the lenght -1 
                                    # append always add a element to the end of a list
                                    position_in_list = len(vehicles_queue_time[i]) - 1
                        else:
                            for i in range(len(position_ligths)):
                                if self.agent.traffic_light.number == i+1: # if light 1, 2, 3, 4
                                    # changes the time that the vehicle is currently waiting on the red light
                                    vehicles_queue_time[i][position_in_list] = round(self.agent.waiting_time, 3)
                                    #print(f"{i}: {vehicles_queue_time[i]}")
...
async def main():
    ...
    f = open("ai-semaphore-project/comparisons/documents/test4.txt", "a")
