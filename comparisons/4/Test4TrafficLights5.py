...
#   there is more than 5 cars on the queue
#   the medium awaiting time of the cars is bigger than 20s
# 
...
class CentralCoordinationAgent(Agent):
    ...
    class CentralCoordinationBehav(CyclicBehaviour):
        ...
        async def run(self):
            turn_on_light = 0
            for i in range(4):
                if ((len(vehicles_queues[i]) > 5) and (sum(vehicles_queue_time[i])/len(vehicles_queues[i]) > 10)):
                    turn_on_light = i+1

            if(turn_on_light == 1 or turn_on_light == 3):
            ...
            elif(turn_on_light == 2 or turn_on_light == 4):
...
async def main():
    ...
    f = open("ai-semaphore-project/comparisons/documents/test5.txt", "a")
